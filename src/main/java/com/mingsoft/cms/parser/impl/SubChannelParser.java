...
public class SubChannelParser extends IParser {
	 ...
	 */
	private final static String SUB_CHANNEL_BEGIN = "\\{ms:subchannel.*?\\}";
	 ...
	 */
	private final static String SUB_CHANNEL_END = "\\{/ms:subchannel\\}";
	 ...
	 */
	private final static String SUB_CHANNEL_TITLE = "\\[field.subtypetitle/\\]";
	 ...
	 * 获取栏目id
	 */
	private final static String SUB_CHANNEL_FIELE_ID="\\[field.subtypeid/\\]";
	 ...
	 *            替换内容
	 */
	public SubChannelParser(String htmlCotent, List<ColumnEntity> newContent, String page) {
		// 在HTML模版中标记出要用内容替换的标签
		String htmlCotents = channelPrplace(htmlCotent, TAB_BEGIN_LIST, SUB_CHANNEL_BEGIN);
		htmlCotents = channelPrplace(htmlCotents, TAB_END_LIST, SUB_CHANNEL_END);
		// 经过遍历后的数组标签
		String newCotent = articleTypeList(htmlCotents, newContent, page);

		super.newCotent = newCotent;
		//System.out.println(super.newCotent);
		super.htmlCotent = htmlCotents;
		//System.out.println(super.htmlCotent);
	}
	 ...
	 *            选中分类样式
	 */
	public SubChannelParser(String htmlCotent, List<ColumnEntity> newContent, String page, int curColumnId, String className) {
	...
	private String articleTypeList(String htmlCotent, List<ColumnEntity> articleTypeList, String webUrl) {
		...
		if (articleTypeList != null && articleTypeList.size() != 0) {
			for (int i = 0; i < articleTypeList.size(); i++) {
				...
				// 替换栏目标题标签
				htmlList = tabContent(htmlList, articleType.getCategoryTitle(), SUB_CHANNEL_TITLE);
				// 替换栏目链接标签
				...
				//替换栏目id标签
				htmlList = tabContent(htmlList, articleType.getCategoryId()+"", SUB_CHANNEL_FIELE_ID);
	...
	public static int channelNum(String html) {
		int channelNumBegin = count(html, SUB_CHANNEL_BEGIN);
	 ...
	 * 定位栏目标签中所有的属性
	 */
	private final static String SUB_CHANNEL_PROPERTY = "\\{ms:subchannel(.*)?\\}";
	...
	public static Map<String, String> channelProperty(String html) {
		...
		String listProperty = parseFirst(html, SUB_CHANNEL_PROPERTY, 1);

	...
	<@ms.contentBody>
		...
		<@ms.contentPanel>
			<@ms.form name="columnForm" isvalidation=true  action="" method="post" >
				...
				<@ms.radio name="columnType" label="${Session.model_title_session?default('栏目')}属性"
						list=columnTypes listKey="id" listValue="name" value="${column.columnType?c?default(1)}" />
				...
				<@ms.select name="columnUrl" width="300" id="columnUrlModel" default="暂无文件"
						list=columnModelUrls  listKey="id" listValue="name" label="内容模版"  value="${column.columnUrl?default('')}"  select2=true/>
				<@ms.checkbox name="status" width="200" list=[{"id":"1","value":"启用"}] listKey="id" listValue="value"
						label="状态" valueList=["${column.status!('1')}"] />
				<#--<@ms.formRow label="测试" width="300">-->

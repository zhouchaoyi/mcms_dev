						...
						<p class="alert alert-info" role="alert" style="margin:0">
				            	<span class="glyphicon glyphicon-pushpin text-lef "></span>
				            		<a  class="alert-link text-lef"  style="margin-left: 12px;">
				            			一键更新所有页面,系统提示“更新中...”请不要刷新页面或点击其他菜单。<font color="red">(注意：生成的文章是起始时间之后的所有文章)</font>
				            		</a>
						</p>
					</div>
	...
	<script type="text/javascript">
		$(function(){
			...
			$("#startUpdate").click(function(){
					...
					$.ajax({
			   			type: "post",
						data: "dateTime=" + $("input[name='updateTime']").val(),
			   			...
			   			success: function(msg){
			     			if(msg.result){
			     				$("#view").show();
			     				alert("更新成功");
			     				$("#startUpdate").html("开始更新").removeAttr("disabled");
			     			}else{
			     				alert("更新失败");
			     			}
			   			}
